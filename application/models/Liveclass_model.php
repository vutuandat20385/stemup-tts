...
class Liveclass_model extends CI_Model {
  ...
  public function validategid($gid,$class_id){
	...
	public function insert_classroom(){
			...
			$class_code =$randomString. $class_id;
			$this->db->where('class_id',$class_id);
			$this->db->update('live_class',array("class_code"=>$class_code));
			if($this->input->post('assigned_groups')){
...
$this->db->update('class_coment', $data); 
   ...
   public function add_student_lc($class_id, $student_id){
  
  		$ins_arr = array("class_id"=>$class_id, "student_id"=>$student_id);
  	    $this->db->insert('liveclass_student_rel', $ins_arr);
			
  }

  public function remove_student_lc($class_id, $student_id){
  
  		$ins_arr = array("class_id"=>$class_id, "student_id"=>$student_id);
  	    $this->db->delete('liveclass_student_rel', $ins_arr); 
