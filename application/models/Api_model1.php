...
$new_password=rand('1111','9999');
 ...
 if($this->config->item('protocol')=="smtp"){
	...
	function question_list3(){
		...
		for($i=0; $i<count($query); $i++){
            ...
            for ($j = 0; $j < count($imgTags[0]); $j++) {
			 ...
			 $origImageSrc=str_replace("..","http://do.stem.vn",$origImageSrc);
	...
	function quiz_assign_1($page,$uids=0,$cid=1,$arr=0,$search=""){
		...
		$parent_id=0;
		if($logged_in['su']==2){		
			$sql = "SELECT A.*, u.first_name,u.last_name FROM 
					(SELECT q.*,a.id,a.startdate,a.enddate,a.aname,a.auid,a.rid,a.uid ,a.status as sta FROM savsoft_quiz q 
		...
		if($logged_in['su']==4){		
			$sql = "SELECT A.* FROM 
					(SELECT B.* ,u1.first_name as first_name FROM 
					(SELECT q.*,a.id,a.startdate,a.enddate,a.aname,a.auid,a.rid, a.uid ,a.status as sta FROM savsoft_quiz q 
    ...
    /*function quiz_assign_1($page,$uids=0){
    	$logged_in=$this->session->userdata('logged_in');
		$uid=$logged_in['uid'];
		$class_id=$logged_in['class_id'];
		$setting_p=explode(',',$logged_in['quiz']);
		...
		if($uids>0){
			$this->db->where('savsoft_assign.uid',$uids);
		}
		...
		$this->db->join('savsoft_assign','savsoft_assign.quid=savsoft_quiz.quidss');
		$this->db->join('savsoft_users','savsoft_users.uid=savsoft_assign.uid');
		$num_quiz=$this->db->get('savsoft_quiz')->num_rows();
		$num_page = ceil($num_quiz/15);
		...
		$this->db->limit(15);
		$this->db->offset(15*$page);
		$data['quiz'] =$this->db->get('savsoft_quiz')->result_array();
		$sql="select DISTINCT  u.*  from savsoft_users u join class_member cm 
				on u.uid = cm.member_id where cm.class_id in ($class_id) 
				and u.su=2 ";
	...
	function class_list_question(){
    	...
    	$uid = $logged_in['uid'];
		$sql="";
		if($logged_in['su']==1 || $logged_in['su']==3){
	...
	function group_list_question(){
		$logged_in=$this->session->userdata('logged_in');
    	$uid = $logged_in['uid'];
		$sql="";
		if($logged_in['su']==1 || $logged_in['su']==3){
			$sql.="SELECT * FROM social_group sg WHERE sg.created_by = $uid ";
		}
		$query=$this->db->query($sql);
		return $query->result_array();
	}
	...
	function class_list_2($quid,$search,$page){
		...
		if($logged_in['su']==3 || $logged_in['su']==1){
			$select = " 
			select DISTINCT sd.did,sd.dataitem_name,sd.dataitem_code from savsoft_dataitem sd 
					INNER JOIN class_metadata cm ON sd.did = cm.class_id 
					INNER JOIN class_member cl ON cl.class_id=cm.class_id 
					WHERE sd.status = 1 and cm.create_by=$uid and cl.member_id !=$uid";
			if($class_ids!=""){
			$class_id = "  and cl.class_id in ($class_ids) ";
		/*	$class= "
		...
		if($search != ''){
			$search = " and sd.dataitem_name like '%$search%' ";
		}else{
			$search = " ";
		}
...
return $query;
	...
	function student_list_2($quid,$search="",$page=0){
    	$logged_in=$this->session->userdata('logged_in');
    	$uid = $logged_in['uid'];
    	//$class_id = $logged_in['class_id'];
		$data_class_id="SELECT u.class_id FROM savsoft_users u WHERE u.uid=$uid";
...
FROM
    (SELECT 
        social_group.sg_name,social_group.sg_code, social_group_joined.sg_id
    FROM
        social_group_joined
...
FROM
    ...
    function sendmail($toemail, $message,$subject="do.stem.vn"){
    	$this->load->library('email');
    	if($this->config->item('protocol')=="smtp"){
			$config['protocol'] = 'smtp';
			$config['smtp_host'] = $this->config->item('smtp_hostname');
    ...
    function unassign_quiz($quid, $uid){
    	...
    	$uid = $logged_in['uid'];
    		...
    		$assignfrom = array(
		  		"uid" => $uid,
		  		"content" => "Đã giao một bài ".$type.' cho nhóm '.$sg_info['sg_name'],
		  		"model" => "quiz",
				"action" => "Assign quiz",
				"click" => "window.location.href = '".site_url()."/quiz/validate_quizs/".$quid."/".$uid."/".$assid."'"
		  	);
    ...
    function assign_to_class($quid, $classid, $startdate, $enddate){
		...
		$dataitem = "select dataitem_name from savsoft_dataitem WHERE did = $classid";
		$dataitem_name= $this->db->query($dataitem)->row();
    	if(count($students) > 0){
    		...
    		$assignfrom = array(
		  		"uid" => $uid,
		  		"content" => "Đã giao một bài ".$type."cho lớp ".$dataitem_name->dataitem_name,
		  		"model" => "quiz",
				"action" => "Assign quiz",
				"click" => "window.location.href = '".site_url()."/quiz/validate_quizs/".$quid."/".$uid."/".$assid."'"
		  	);
...
//        }
	...
	function assign_question_gr($qid,$group){
		$logged_in=$this->session->userdata('logged_in');
    	$assid = $logged_in['uid'];
		$this->db->where('sg_id',$group);
		$group_name = $this->db->get('social_group')->row_array()['sg_name'];
		$sql = " SELECT sgj.uid FROM social_group_joined sgj INNER JOIN social_group sg ON sgj.sg_id=sg.sg_id WHERE sgj.uid!=$assid and sgj.sg_id=$group ";
		$students = $this->db->query($sql)->result_array();
		$this->db->where('qid', $qid);
		$this->db->where('assid', $assid );
		...
		if($data > 0){
			...
			foreach($students as $student){
				...
				$assignfrom = array(
				"uid" => $assid,
				"content" => "Đã giao một câu hỏi cho nhóm ".$group_name." của bạn!",
				"model" => "savsoft_qbank",
				"action" => "Assign question group",
				'click' => "question_ass_gr(".$qid.",".$assid.",".$group.")"
				);
				$this->db->insert('notify',$assignfrom);
				$nid = $this->db->insert_id();
	...
	function anwser_ass_cl($qid, $assid, $answer,$class_id){
		...
		$assignfrom = array(
	  		"uid" => $uid,
	  		"content" => "Đã trả lời câu hỏi của bạn đố cho lớp ".$class_name."!",
	  		"model" => "savsoft_qbank",
	  		"action" => "Assign question",
	...
	function anwser_ass_gr($qid, $assid, $answer,$group_id){
		$logged_in=$this->session->userdata('logged_in');
    	$uid = $logged_in['uid'];
		$this->db->where('qid', $qid);
		$this->db->where('assid', $assid);
		...
		$answerdata = array('answer' => $answer);
		$this->db->update('savsoft_qassign', $answerdata);
        $this->db->where('sg_id',$group_id);
		$group_name = $this->db->get('social_group')->row_array()['sg_name'];
		$assignfrom = array(
	  		"uid" => $uid,
	  		"content" => "Đã trả lời câu hỏi của bạn đố cho nhóm ".$group_name."!",
	  		"model" => "savsoft_qbank",
	  		"action" => "Assign question",
	  		'click' => "answer_ass_gr(".$qid.",".$assid.",".$answer.",".$group_id.")"
	  	);
	  	$this->db->insert('notify',$assignfrom);
	  	$nid = $this->db->insert_id();
	...
	function check_answer_gr($qid,$assid,$group_id){	
		$logged_in=$this->session->userdata('logged_in');
		$uid = $logged_in['uid'];
		$this->db->where('uid', $uid);
		$this->db->where('qid', $qid);
  ...
  function dataforvuidetail($cname){
  	...
  	for($i=0; $i<count($data); $i++){
        ...
        for ($j = 0; $j < count($imgTags[0]); $j++) {
		 ...
		 $origImageSrc=str_replace("..","https://do.stem.vn",$origImageSrc);
		 if($origImageSrc){
			$data[$i]['img']=$origImageSrc;
		 }
         else{
			 if($data[$i]['background_template']!=0){
				 $data[$i]['img']="https://do.stem.vn/upload/background/".$data[$i]['background_template'].".jpg";
			 }
			 else{
				 $data[$i]['img']="https://do.stem.vn/upload/background/".rand(1,20).".jpg";
			 }
