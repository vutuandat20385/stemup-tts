...
<html lang="en">
  <head>
	...
	<script src="<?php echo base_url('js/manage_group.js') ?>"></script>
	 <script src="<?php echo base_url('js/manage_results.js');?> "></script>
	<script src="<?php echo base_url('js/addclass.js');?>"></script>
	<script src="<?php echo base_url('js/group.js');?>"></script>
	<script src="<?php echo base_url('js/manage_results1.js');?>"></script>
 ...
 <div class="modal fade" id="changepwdModal" role="dialog">
      ...
      <?php if($su==4){?>
			...
			<div class="modal-dialog">	 
				 <div class="modal-content">
					...
					<div class="modal-body">
					   <form method="post" action="<?php echo site_url('/home_user/add_child');?>">
							 <input type="text" class="form-control form-TK1 col-xs-12"  name="student_code" style="width:450px" placeholder="Mã học sinh"/>
	...
	<section class="row">
		 ...
		 <div class="box-bor MB20 visible-xs-block">
		  ...
		  <div class="box-bor MB20 hidden-xs" style="text-align: center;">
			  ...
			  <li class="pull-right MR10">
				<div class="btn-group">
				  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
					<i class="fas fa-cog MR10"></i>Cài đặt<span class="caret"></span>
				  </button>
					...
					<div class="box-bor MB20">
						<div role="tabpanel" id="tabs">
						  <ul class="nav nav-tabs" role="tablist">
								<li role="presentation" class="active"><a href="#home1"  id="text-tabs" data-toggle="tab" role="tab" aria-controls="tab1">Kết quả bài trắc nghiệm</a></li>
								
								<li role="presentation"><a href="#home3" data-toggle="tab" role="tab" aria-controls="tab2" id="text-tabs3" style="">Kết quả câu hỏi đố</a></li>
								<!--	<li role="presentation" class="dropdown"><a href="#" id="tabDropOne1" class="dropdown-toggle" data-toggle="dropdown" role="tab" aria-controls="tab3" aria-haspopup="true" aria-expanded="false">Giao bài<span class="caret"></span></a>
						  ...
						  <div id="tabContent1" class="tab-content">
							<div role="tabpanel" class="tab-pane fade pad in active" id="home1">
							     <div class="data_rs">
							      <table class="table table-bordered">
									  <tr style="background-color: rgb(233, 235, 238);">
											...
											<?php if($su!=2){ ?>
											<th>
											<select style="width:60px" onchange="drawname_mng_qt2(this)">
											    <option value="0">Họ tên</option>
												<option value="0">Tất cả</option>
												<?php foreach($users as $resul){ ?>
											...
											Bài trắc nghiệm
											<span>
											<select style="float:right;width:60px" onchange="drawct_mng_qt2(this)">
											    <option value="1">Môn</option>
												<option value="1">Tất cả</option>
												<?php foreach($mapping_category1 as $key=>$val){ ?>
											...
											<th>
											<input id="search_rs" style="min-width:40px; width:80%" onkeyup="drawsearch_rs(this,event)"> 
											<span><i class="pointer fas fa-search" onclick="drawsearch_rs_btn()"></i></span>
											</th>
									  ...
									  <tr>
										...
										<td>
											<?php echo $res['quiz_name']?>
										</td>
										<td>
											<?php echo $res['result_status']?>
										</td>
										<td>
											<?php echo number_format($res['percentage_obtained'],0);?>%
										</td>
										<td>
											<a href="<?php echo site_url('result/view_result/'.$res['rid'])?>" >Chi tiết </a>
										</td>
									  </tr>
									  <?php } ?>
								  ...
								  <input type="text" id="inf_cid2" value="1">
								  <input type="text" id="inf_uid2" value="0">
								  </div>
							</div>
							...
							<div class="data_rs1">
							      <table class="table table-bordered">
									  <tr style="background-color: rgb(233, 235, 238);">
											...
											<?php if($su!=2){ ?>
											<th>
											<select style="width:60px" onchange="drawname_mng_qt(this)">
											    <option value="0">Họ tên</option>
												<option value="0">Tất cả</option>
												<?php foreach($users as $resul){ ?>
												<option value="<?php echo $resul['uid'];?>"> <?php echo $resul['first_name'];?></option>
											...
											Câu hỏi đố
											<span>
											<select style="float:right;width:60px" onchange="drawct_mng_qt(this)">
											...
											<th>
											<input id="search_rs1" style="min-width:40px; width:80%" onkeyup="drawsearch_rs1(this,event)"> 
											<span><i class="pointer fas fa-search" onclick="drawsearch_rs1_btn()"></i></span>
											</th>
									  ...
									  <tr>
										...
										<td>
											<?php $a = chr($res['option_choice']+65);
											echo $a;?>
										</td>
										<td>
											<?php $b = chr($res['option_correct']+65);
											echo $b;?>
										</td>
										<td>
								  ...
								  <P>Đang xem <span id="beginrs1"><?php echo min($limit*$page+1,$num_result1);?></span> 
								  đến <span id="endrs1"><?php echo min($limit*($page+1),$num_result1);?></span> 
								  trong tổng số <span id="totalrs1"><?php echo $num_result1;?></span> kết quả<p>
								  <center>
								  <ul class="pagination listpage pagers1">
								    <?php if($num_page1>6){?>
								    <li class="page-item active" onclick="drawpage_rs1(0)"><a class="page-link">1</a></li>
								    <li class="page-item" onclick="drawpage_rs1(1)"><a class="page-link">2</a></li>
									<li class="page-item" onclick="drawpage_rs1(2)"><a class="page-link">3</a></li>
									<li class="page-item" onclick="drawpage_rs1(3)"><a class="page-link">4</a></li>
									<li class="page-item" onclick="drawpage_rs1(4)"><a class="page-link">5</a></li>
									  <?php if($num_page1>7){ ?>
									  <li class="page-item"><a class="page-link">...</a></li>
									  <?php } ?>
									  <li class="page-item" onclick="drawpage_rs1(<?php  echo $num_page1-1 ?>)"><a class="page-link"><?php echo $num_page1 ?></a></li>
									<?php }else{?>
									  <li class="page-item active" onclick="drawpage_rs1(0)"><a class="page-link">1</a></li>
									  <?php for($i=1; $i<$num_page1; $i++){?>
										<li class="page-item" onclick="drawpage_rs1(<?php  echo $i ?>)"><a class="page-link"><?php  echo $i+1 ?></a></li>
									  <?php }?>
									<?php }?>
								  ...
								  <div style="display:none">
								  <input type="text" id="inf_search1" value="">
								  <input type="text" id="inf_page1" value="0">
								  <input type="text" id="inf_limit1" value="10">
								  <input type="text" id="inf_uid1" value="0">
								  <input type="text" id="inf_cid1" value="0">
								  </div>
							</div>
