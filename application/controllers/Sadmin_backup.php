...
class Sadmin extends CI_Controller {
  ...
  //           $config = array();
	...
	function manage_news(){
		...
		$config['base_url'] = base_url() . 'index.php/sadmin/manage_news';
		$config['total_rows'] = $this->new_stemup_model->get_all_news();
		$config['per_page'] = $limit;
		$config["uri_segment"] = 3;
		$config['use_page_numbers'] = TRUE;
		$config['reuse_query_string'] = TRUE;
		$data['list_news']=$this->new_stemup_model->get_news_limit($start,$limit);
	...
	function edit_news(){
		$user = $this->session->userdata("logged_in");
		$nid = ($this->uri->segment(3)) ? ($this->uri->segment(3)) : 0;
		$data['news'] = $this->sadmin_model->get_new($nid);
		$related_news = $data['news']['related_news'];
		
		$data['related_news'] = $this->sadmin_model->get_list_chon_v2($data['news']['related_news']);

		$data['category_news'] = $this->sadmin_model->get_category_news();
		$data['related_news_list'] = $this->sadmin_model->get_related_news_list($nid);
		
		$arr_related_news = explode(",",$related_news);
		foreach($data['related_news_list'] as $k=>$val){
			if(in_array($val['id'],$arr_related_news)){
				$data['related_news_list'][$k]['check']=1;
			}else
				$data['related_news_list'][$k]['check']=0;
		}
