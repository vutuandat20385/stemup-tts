...
function attempt($rid,$selected_lang=0){
	...
	if($rid != $srid){
		...
		*/
		...
		for($i=0; $i<count($data['questions']); $i++){
			for($j=0; $j<count($arr_video); $j++){
				if($data['questions'][$i]['qid']==$arr_video[$j]['qid']){
					$data['questions'][$i]['question'] = str_replace('</iframe>','</iframe><span class="text-intro">Bạn hãy xem video trên và trả lời câu hỏi:</span>',$arr_video[$j]['question']);
				};
			}
		}

		//xu ly video link
		$v_link_start = strpos($arr_video[0]['question'],'src');
		$v_link_end = strpos($arr_video[0]['question'],'" width=');
		$data['v_link_full_1'] = substr($arr_video[0]['question'],$v_link_start,$v_link_end-8);

		if(strpos($data['v_link_full_1'],'?')>0){
			$data['v_link_full_2'] = 'id="video" '.$data['v_link_full_1'].'&enablejsapi=1';
		}else{
			$data['v_link_full_2'] = 'id="video" '.$data['v_link_full_1'].'?enablejsapi=1';
		}
		// $arr_video[0]['question'] = str_replace( $v_link_full_1, $v_link_full_2,$arr_video[0]['question']);
		...
		$data['count_questions'] = sizeof($data['questions']);
...
public function loadoption1($qid, $qk, $count_question,$rid){

	$data= $this->quiz_model->loadoption1($qid, $qk, $count_question,$rid);
	header('Content-Type: application/json');
	echo json_encode($data);
}
...
function Quiz_file(){
	...
	$data['i'] = $this->input->post('sofile');
	$data['result'] = $this->quiz_model->Quiz_file_model($start);
	$this->load->view("stemup/xuatfile_quiz",$data);
}
