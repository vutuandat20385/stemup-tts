...
class Home_user extends CI_Controller {
	...
	public function quiz_assign_1($page,$uid,$cid,$arr){
		...
		log_message('error',json_decode($data['assign']));
		header('Content-Type: application/json');
		echo json_encode($data);
	}
	...
	public function get_data_std($quid){
		...
		if($user){
			...
			$inp['np_std'] = ceil($inp['nb_students']/3);
			header('Content-Type: application/json');
			echo json_encode($inp);
		}
	}
	...
	function assign_quiz(){
		...
		if($user){
			...
			$data['results']= $this->api_model->list_question_assign("",0,5,0,0);
			$data['results1']= $this->api_model->list_question_assign2("",0,5,0,0);
	...
	function get_data_result2(){
		...
		if($user){
			...
			$inp['search']=htmlentities($inp['search'], ENT_COMPAT, 'UTF-8');
			$inp['result']= $this->api_model->list_question_assign($inp['search'],$inp['cid'],$inp['limit'],$inp['uid'],$inp['page']); 
			$inp['num_result']= $this->api_model->num_question_assign($inp['search'],$inp['cid'],$inp['uid']); 
			$inp['num_page']=ceil($inp['num_result']/$inp['limit']);
			$inp['users']=$this->api_model->student_list_1($inp['cid'],$inp['uid']);
			header('Content-Type: application/json');
			echo json_encode($inp);
	...
	function get_data_result3(){
		...
		if($user){
			...
			$inp['search']=htmlentities($inp['search'], ENT_COMPAT, 'UTF-8');
			$inp['result1']= $this->api_model->list_question_assign2($inp['search'],$inp['cid'],$inp['limit'],$inp['uid'],$inp['page']); 
			$inp['num_result1']= $this->api_model->num_question_assign2($inp['search'],$inp['cid'],$inp['uid']); 
			$inp['num_page1']=ceil($inp['num_result1']/$inp['limit']);
...
// 	        $data['quiz_list'] = $this->kns_model->get_quiz_list();
	        ...
	        $total_records = count($this->kns_model->get_quiz_list());
	        
	        $conf['base_url'] = base_url('index.php/home_user/kynangsong/');
