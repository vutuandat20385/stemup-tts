...
class Sadmin extends CI_Controller {
	...
	public function logout() {
		...
		header('Location: '.base_url().'index.php/sadmin/login');
	}
	...
	function news(){
      ...
      //  var_dump($user['id'])
		if($user['su'] == 1){
			...
			$data['list_news'] = $this->sadmin_model->list_news(0,30);
	...
	function manage_news(){
		...
		$total_row = $this->sadmin_model->num_news();
		$config["total_rows"] = $total_row;
		$config["per_page"] = 6;
		$data['limit']=6;
		...
		$data['page']=$page;
		$data["list_news"] = $this->sadmin_model->list_news( $page,$config["per_page"]);
