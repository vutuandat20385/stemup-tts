		...
		success: function(results){
			...
			if(results.assign.quiz.length > 0){
			    for (var i = results.assign.quiz.length-1; i >=0; i--) {
					...
					if(results.user.su == "2"){
						html += '<i class="color-blue far fa-user"></i> Người giao: <span class="text-strong">' + results.assign.quiz[i].aname +'</span>';
					}else{
						html += '<i class="color-blue far fa-user"></i> Học sinh: <span class="text-strong">' + results.assign.quiz[i].first_name + ' '+ results.assign.quiz[i].last_name +'</span>';
					}
					...
					if(results.assign.quiz[i].sta == 2){
					html+='<br><i class="color-blue fas fa-chart-bar"></i> Kết quả: <span class="text-strong">'+ Math.ceil(results.assign.quiz[i].percentage_obtained) +'% </span><br>';
					}
					html+='</div>';
					...
					if(results.assign.quiz[i].sta == 1){
						...
						html+='<i class="color-blue fas fa-trophy"></i> Thưởng tối đa: <span class="text-strong">'+ results.assign.quiz[i].reward_point +' sao </span>';
			    	}else{
						
			    		html+='<i class="color-blue fas fa-address-book"></i> Loại bài: <span class="text-strong">'+ loai_bai +'</span><br>';
						if(results.user.su == "4"){
						html+='<i class="color-blue fas fa-tags"></i> Trừ giao bài: <span class="text-strong">'+ results.assign.quiz[i].price +' sao</span><br>';
						}
						
						html+='<i class="color-blue fas fa-trophy"></i>';
							switch (results.user.su){
								case '2':
									html+=' Được thưởng: ';
									break;
								case '4':
						...
						html+= '<span class="text-strong">'+ Math.ceil((results.assign.quiz[i].percentage_obtained * results.assign.quiz[i].reward_point)/100) +' sao</span><br>';
						if(results.user.su == "4"){
							html+='<i class="color-blue fas fa-star"></i> Sao còn lại: <span class="text-strong">';
...
function quizAssignTo_done(page,uidz,cid,arr,search){
	...
	$.ajax({
	 	type: "POST",
		data : {search:search},
		url: base_url + "index.php/home_user/quiz_assign_2/"+page +"/"+ uidz +"/"+ cid +"/"+ arr ,
		success: function(results){
			console.log(results);
			console.log(results.user);
			var m= ["Tổ hợp","","","Toán","Vật Lý","Hóa học","Địa lý","Tin Học","Sinh học","Khoa học","Lịch sử","Công nghệ","Tiếng Anh","Thiên văn - vũ trụ","Robot","Môi trường","Sức khỏe","Văn học","Tiếng Việt","Xã hội","Test IQ","Giáo dục công dân","Tự nhiên và xã hội"];
			...
			if(results.assign.quiz.length > 0){
			    for (var i = results.assign.quiz.length-1; i >=0; i--) {
			    	var html = '<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="card_done' + results.assign.quiz[i].id + '">';
			    	if(results.assign.quiz[i].sta == 1){
			    		html += '<div class="offer offer-danger" title="'+results.assign.quiz[i].quiz_name+'"><div class="shape"><div class="shape-text" title="Chưa làm!"><i class="fa fa-ban"></i></div></div>';
			    	}else{
			    		html += '<div class="offer offer-success" title="'+results.assign.quiz[i].quiz_name+'"><div class="shape"><div class="shape-text" title="Đã hoàn thành!"><i class="fa fa-check"></i></div></div>';
...
function quizAssignTo_undone(page,uidz,cid,arr,search){
	...
	$.ajax({
	 	type: "POST",
		data : {search:search},
		url: base_url + "index.php/home_user/quiz_assign_3/"+page +"/"+ uidz +"/"+ cid +"/"+ arr ,
		success: function(results){
			console.log(results);
			console.log(results.user);
			var m= ["Tổ hợp","","","Toán","Vật Lý","Hóa học","Địa lý","Tin Học","Sinh học","Khoa học","Lịch sử","Công nghệ","Tiếng Anh","Thiên văn - vũ trụ","Robot","Môi trường","Sức khỏe","Văn học","Tiếng Việt","Xã hội","Test IQ","Giáo dục công dân","Tự nhiên và xã hội"];
			...
			if(results.assign.quiz.length > 0){
			    for (var i = results.assign.quiz.length-1; i >=0; i--) {
			    	var html = '<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="card_undone' + results.assign.quiz[i].id + '">';
			    	if(results.assign.quiz[i].sta == 1){
			    		html += '<div class="offer offer-danger" title="'+results.assign.quiz[i].quiz_name+'"><div class="shape"><div class="shape-text" title="Chưa làm!"><i class="fa fa-ban"></i></div></div>';
			    	}else{
			    		html += '<div class="offer offer-success" title="'+results.assign.quiz[i].quiz_name+'"><div class="shape"><div class="shape-text" title="Đã hoàn thành!"><i class="fa fa-check"></i></div></div>';
