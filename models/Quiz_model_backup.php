...
Class Quiz_model extends CI_Model
{
 ...
 function insert_quiz_system($quiz_name,$qids,$noq,$class,$day,$lid,$level_hard,$set,$mpoint, $reading_mcq,$video_mcq,$reading,$reading_title,$img_reading,$video_title,$img_video){
	 $qidss= $qids;
	 if($reading_mcq){
		 $qidss.= ",".$reading_mcq;
	 }
...
*/
	 ...
	 for($k=0; $k<count($qt_list); $k++){
		$qt_list[$k]['question'] =str_replace('src="..', 'src="https://do.stem.vn',$qt_list[$k]['question']);
...
function get_qcl($quid){
 ...
 function get_quiz_block($quid){
	...
	if($user){
		$html.= '<h3 class="mt-0">Bài trắc nghiệm: <a href="'.site_url('quiz/validate_quiz/'.$quid).'">'.$quiz_fun['quiz_name'].'</a></h3>';
	}
	else{
    ...
    foreach($quiz_fun['question'] as $k=>$qf){
		if($user){	
			...
			$html.='<a class="col-md-3 col-xs-6 mb-10" href="'.site_url('quiz/validate_quiz/'.$quiz_fun['quid']).'"><img class="img-responsive img_quiz" src="'. str_replace('..','https://do.stem.vn', $qf['img']).'" alt=""></a>';
		}
		else{
    ...
    foreach($quiz_fun['question'] as $k=>$qf){
		if($k<2){
			if($user){
				...
				$html.='<a class="col-md-3 col-xs-6 mb-10" class="pointer"  href="'.site_url('page/quiz/'.$quiz_fun['permalink']).'><img class="img-responsive img_quiz" src="'.str_replace('..','https://do.stem.vn', $qf['img']).'" alt=""></a>';
			}
			 ...
			 if($qr){
				 ...
				 if(count($imgTags[0])>0){
					 if(strpos($origImageSrc, "latex.codecogs.com")===false){
						$query[$k]['img']=str_replace("..",'https://do.stem.vn',$origImageSrc);
					 }
					 else{
...
if($this->config->item('allow_result_email')){
 ...
 function like($quid){
  ...
  function loadoption($qid, $qk,$count_question){
	  ...
	  $message.='<span id="optA1" type="text" class="form-control optvalmb pointer bo-input-right" name="option[]"   readonly style="color: transparent; text-shadow: 0 0 0 #444;resize: none; height:30px;padding-bottom:0px;overflow: hidden;">'; 
	  $message.='<table>';
	  $message.='<tr><td><b> A:</b> </td>';
	 $message.='<td>'.html_entity_decode($data[0]['q_option']).'</td>';
	$message.='</tr></table>';
	$message.='</span>';
	$message.='</div>';					
	$message.='</label>';
	...
	$message.='<div class="input-group mobile-text-opt">';
	$message.='<span class="input-group-addon opt-mb2 bo-input-left"></span>';
	$message.='<span id="optB1" type="text" class="form-control optvalmb pointer bo-input-right" name="option[]"   readonly style="color: transparent; text-shadow: 0 0 0 #444;resize: none; height:30px;padding-bottom:0px;overflow: hidden;"> ';
	$message.='<table>';
	$message.='<tr><td><b> B:</b> </td>';
	$message.='<td>'.html_entity_decode($data[1]['q_option']).'</td>';
	$message.='</tr></table>';
	$message.='</span>';
	$message.='</div>';									
	$message.='</label>';
	...
	$message.='<div class="input-group mobile-text-opt" >';
	$message.='<span class="input-group-addon opt-mb2 bo-input-left"></span>';
	$message.='<span id="optC1" type="text" class="form-control optvalmb pointer bo-input-right" name="option[]"   readonly style="color: transparent; text-shadow: 0 0 0 #444;resize: none; height:30px;padding-bottom:0px;overflow: hidden;"> ';
	$message.='<table>';
	$message.='<tr><td><b> C:</b> </td>';
	$message.=' <td>'.html_entity_decode($data[2]['q_option']).'</td>';
	$message.='</tr></table>';
	$message.='</span>';
	$message.='</div>';							
	$message.='</label>';
	...
	$message.=' <td>'.html_entity_decode($data[3]['q_option']).'</td>';
	$message.='</tr></table>';
	$message.='</span>';
	$message.='</div>';							
	$message.='</label>';
