...
Class Account_model extends CI_Model
{
 ...
 function account_list_import(){
	  $logged_in=$this->session->userdata('logged_in');
		$this->db->where("account_id",2);
		$this->db->or_where("account_id",3);
	 // $this->db->limit($this->config->item('number_of_rows'),$limit);
		//$this->db->order_by('account_id','desc');
		$query=$this->db->get('account_type');
		return $query->result_array();
 ...
 function get_edit_account($account_id){
      ...
      if($this->input->post('social_group')){
       $userdata['social_group']=implode(',',$this->input->post('social_group'));
       }else{
       $userdata['social_group']="";
       }  
   ...
   if($this->input->post('assignment')){
       $userdata['assignment']=implode(',',$this->input->post('assignment'));
       }else{
       $userdata['assignment']="";
       }  
