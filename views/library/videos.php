  	  ...
  	  <aside class="col-md-7" >
	      <div class="box-bor" style="margin-bottom:20px">
			...
			<?php if(strpos($vid['content'], "facebook")!==false){?>
				 <div class="video-container2" id="video-if-<?php echo $video['lib_id'] ?>"> 
					<?php echo $video['content'] ?>
				 </div>
			<?php } else { ?>
				<div class="video-container" id="video-if-<?php echo $video['lib_id'] ?>"> 
					<?php echo $video['content'] ?>
				 </div>
			<?php } ?>
			 <center>
			    <div style="margin-bottom:40px">
					<h4><i><?php echo $video['description'] ?></i></h4>
					<h4><i style="float:right">Theo <?php echo $video['source'] ?></i></h4>
					</div>
				<div>
				<?php if($su==1|| $su==3 || $su==6){ ?>
					<p><input type="button" class="btn btn-success" value="Tạo câu hỏi" onclick="create_question_video(<?php echo$video['lib_id'] ?>)"></p>
				<?php } ?>
				</div>
			</center>	
		  </div>
